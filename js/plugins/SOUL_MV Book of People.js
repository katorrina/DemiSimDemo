...
/*:
 * @plugindesc 2.8 Displays a detailed status of the people you've met in your adventure.
 * @author Soulpour777 - soulxregalia.wordpress.com

 *
 * @param Show on Menu
 * @desc (true / false) (Default - can be changed with a plugin command)
 * @default true
 *
 * @param Show PersonalName
 * @desc (true / false) Do you want to show the personal name of the actor in the Book of People plugin?
 * @default true
 *
 * @param Show Status
 * @desc (true / false) Do you want to show the status of the actor in the Book of People plugin?
 * @default true
 *
 * @param Show Age
 * @desc (true / false) Do you want to show the age of the actor in the Book of People plugin?
 * @default true
 *
 * @param Show Job
 * @desc (true / false) Do you want to show the job of the actor in the Book of People plugin?
 * @default true
 *
 * @param Show Place
 * @desc (true / false) Do you want to show the place of the actor in the Book of People plugin?
 * @default true
 *
 * @param Show SkillName
 * @desc (true / false) Do you want to show the skillname of the actor in the Book of People plugin?
 * @default true
 *
 * @param Show Bio
 * @desc (true / false) Do you want to show the biography of the actor in the Book of People plugin?
 * @default true
 *
 * @param Show Description
 * @desc (true / false) Do you want to show the description of the actor in the Book of People plugin?
 * @default true
 *
 * @param Show MottoName
 * @desc (true / false) Do you want to show the motto name of the actor in the Book of People plugin?
 * @default true
 *
 * @param Show Motto
 * @desc (true / false) Do you want to show the motto of the actor in the Book of People plugin?
 * @default true
 *
...
To create a person's record, you must create it inside the items
tab. It should be a regular item, so it would be accessible.
Inside the item's note tag, you place the following note
tags shown above for each information you want to retain
the actor has.

Plugin Changes on Version 2.8

As there have been many errors with this plugin, I have the care
to check and it has a lot of flaws, even if it is a very usable
one. So I have to check each of them and set them properly
and make it more flexible as it should be.

Changes Made:

I. Add / Disclose access in the menu at will.
The developer can now use a plugin command to change the accessibility of the
plugin on the menu at will.

Plugin Command:

BookOfPeople MenuAccess x

where x is either true or false. By true, it shows on the menu, by false
it won't show on the menu.

II. Show / Hide Each Section

Each section is now easily changed whether you want to see them or not
from the plugin parameters. This way, you can only show what you want 
as information inside the BookOfPeople menu.

III. Fixed the portrait errors. The portrait only shows if it is
inside the book.
...
(function() {
    ...
    SOUL_MV.BookOfPeople.showStatus = PluginManager.parameters('SOUL_MV Book of People')['Show Status'] === "true" ? true : false;
    SOUL_MV.BookOfPeople.showAge = PluginManager.parameters('SOUL_MV Book of People')['Show Age'] === "true" ? true : false;
    SOUL_MV.BookOfPeople.showBio = PluginManager.parameters('SOUL_MV Book of People')['Show Bio'] === "true" ? true : false;
    SOUL_MV.BookOfPeople.showJob = PluginManager.parameters('SOUL_MV Book of People')['Show Job'] === "true" ? true : false;
    SOUL_MV.BookOfPeople.showMotto = PluginManager.parameters('SOUL_MV Book of People')['Show Motto'] === "true" ? true : false;
    SOUL_MV.BookOfPeople.showDesc = PluginManager.parameters('SOUL_MV Book of People')['Show Description'] === "true" ? true : false;
    SOUL_MV.BookOfPeople.showmottoName = PluginManager.parameters('SOUL_MV Book of People')['Show MottoName'] === "true" ? true : false;
    SOUL_MV.BookOfPeople.showPlace = PluginManager.parameters('SOUL_MV Book of People')['Show Place'] === "true" ? true : false;
    SOUL_MV.BookOfPeople.showskillname = PluginManager.parameters('SOUL_MV Book of People')['Show SkillName'] === "true" ? true : false;
    SOUL_MV.BookOfPeople.showpersonalname = PluginManager.parameters('SOUL_MV Book of People')['Show PersonalName'] === "true" ? true : false;
    ...
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
        ...
        if (command === 'BookOfPeople') {
            ...
            case 'MenuAccess':
                if (args[1] === 'true') {
                    $gameSystem._accessOnMenu = true;
                }
                if (args[1] === 'false') {
                    $gameSystem._accessOnMenu = false;
                }
            }
        }
    };

    SOUL_MV.BookOfPeople.gameSystem_initialize = Game_System.prototype.initialize;
    Game_System.prototype.initialize = function() { 
        SOUL_MV.BookOfPeople.gameSystem_initialize.call(this);
        this._accessOnMenu = SOUL_MV.BookOfPeople.showOnMenu;
    }
	...
	Window_MenuCommand.prototype.addOriginalCommands = function() {
		...
		if($gameSystem._accessOnMenu) {
            this.addCommand('Book of People', 'bookofpeopleX', true);
        }
	...
	Scene_Menu.prototype.createCommandWindow = function() {
	    ...
	    if($gameSystem._accessOnMenu)this._commandWindow.setHandler('bookofpeopleX',    this.callBookScene.bind(this));
    ...
    SOUL_MV_PeopleBookStatus.prototype.refresh = function() {
        ...
        if ($gameSystem.isInPeopleBook(item)) {
            ...
            this.portrait.bitmap = null;
        }
         
        if (SOUL_MV.BookOfPeople.showpersonalname) {
